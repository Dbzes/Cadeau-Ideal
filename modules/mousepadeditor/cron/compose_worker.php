<?php
/**
 * Worker cron — traite la queue de composition HD en arrière-plan
 * Lancé toutes les 30s via cron Docker
 * Usage : php /var/www/html/modules/mousepadeditor/cron/compose_worker.php
 */
...
$lockFile = sys_get_temp_dir() . '/mpe_compose_worker.lock';
$fp = fopen($lockFile, 'w');
if (!flock($fp, LOCK_EX | LOCK_NB)) {
    exit(0); // Déjà en cours
}

$db = Db::getInstance();

// Prendre les jobs pending (max 3 par exécution)
$jobs = $db->executeS('
    SELECT * FROM ' . _DB_PREFIX_ . 'mpe_compose_queue
...
foreach ($jobs as $job) {
    ...
    $db->update('mpe_compose_queue', ['status' => 'processing'], 'id_queue = ' . $id);
    ...
    try {
        ...
        // Copier les fichiers HD dans /upload/
        $dest = _PS_UPLOAD_DIR_ . $hash;
        $cleanFile = $result['cleanFile'];
        $previewFile = $result['previewFile'];

        // Fichier principal = clean HD (pour impression)
        copy($cleanFile, $dest);

        // _small = thumbnail 1200px à partir du preview (avec overlay)
        ...
        // Nettoyage des fichiers temporaires dans previews/
        @unlink($cleanFile);
        @unlink($previewFile);

        $db->update('mpe_compose_queue', [
            'status' => 'done',
...
// --- Fonctions de composition (extraites de compose.php) ---
